...
{
  public:
    Array(size_t n, bool usePinned = true) : nSize(n),
                                             hPtr(0),
                                             dPtr(0),
    ...
    Array(T *hostMem, size_t n) : nSize(n),
                                  hPtr(hostMem),
                                  dPtr(0),
    ...
    T *hostPtr()
    {
        if (isDeviceValid && !isHostValid)
            copyDeviceToHost();
        else if (!isHostAlloced)
            allocateHost();
        else
            assert(isHostValid);
        isDeviceValid = false;
    ...
    {
        if (loc == host)
            return readOnlyHostPtr();
        else
            return readOnlyDevicePtr();
    }
    ...
    {
        if (loc == host)
            return writeOnlyHostPtr();
        else
            return writeOnlyDevicePtr();
    }
  ...
  protected:
    virtual void allocateHost() const
    {
        assert(!isHostAlloced);
#ifndef HEMI_CUDA_DISABLE
        if (isPinned)
    ...
    virtual void allocateDevice() const
    {
#ifndef HEMI_CUDA_DISABLE
        assert(!isDeviceAlloced);
    ...
    virtual void copyHostToDevice() const
    {
#ifndef HEMI_CUDA_DISABLE
        assert(isHostAlloced);
        if (!isDeviceAlloced)
